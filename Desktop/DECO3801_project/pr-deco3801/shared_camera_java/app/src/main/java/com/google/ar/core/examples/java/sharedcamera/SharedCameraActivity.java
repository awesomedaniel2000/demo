...
import com.google.ar.core.examples.java.common.helpers.BoundingBoxViewHelper;
...
//            setRepeatingCaptureRequest();
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    cpuImageReader = ImageReader.newInstance(1080, 1920, ImageFormat.JPEG, 1);
    ...
    RelativeLayout rootLayout = findViewById(R.id.rootRelativeLayout);
    ...
    // Create an instance of BoundingBoxViewHelper
    BoundingBoxViewHelper boundingBoxView = new BoundingBoxViewHelper(this);
    ...
    boundingBoxView.setLayoutParams(layoutParams);

    // Initially set it to invisible
    boundingBoxView.setVisibility(View.VISIBLE);

    // Add the BoundingBoxView to the RelativeLayout
    RelativeLayout rootView = findViewById(R.id.rootRelativeLayout);
    rootView.addView(boundingBoxView);
  ...
  private void captureImage() {
    ...
    try {
      ...
      captureBuilder.addTarget(cpuImageReader.getSurface());

      // Additional capture settings if needed
  ...
  private void saveImageToFile(byte[] jpegData) {
    File file = new File(getExternalFilesDir(null), "captured_image.JPEG");
    try (FileOutputStream output = new FileOutputStream(file)) {
      output.write(jpegData);
      Log.d(TAG, "Image saved: " + file.getAbsolutePath());
      Toast.makeText(SharedCameraActivity.this, "Image Saved!", Toast.LENGTH_SHORT).show();
    ...
    try {
      ...
      if (image != null) {
        if (image.getFormat() != ImageFormat.JPEG) {
          Log.d(TAG, "Image format is not JPEG: " + image.getFormat());
          return; // Exit if the format isn't JPEG
        }
        ...
        // Log the byte size
        Log.d(TAG, "Image byte size: " + bytes.length);
      }
    } finally {
      if (image != null) {
        Log.d(TAG, "Image byte size: ");
